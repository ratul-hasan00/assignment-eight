...
const AppDetails = () => {
  ...
  // Track if this app is installed
  const [installed, setInstalled] = useState(false);

  // Check localStorage on load
  useEffect(() => {
    const apps = JSON.parse(localStorage.getItem("installedApps")) || [];
    setInstalled(apps.some((app) => app.id === singleApp.id));
  }, [singleApp.id]);

  const handleInstall = () => {
    const apps = JSON.parse(localStorage.getItem("installedApps")) || [];
    if (!apps.some((app) => app.id === singleApp.id)) {
      apps.push(singleApp);
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 mt-10 mb-10 max-w-[1250px] mx-auto border-b border-gray-200 pb-10 px-4 sm:px-6 lg:px-8">
        <div className="flex-shrink-0">
          <img className="w-full max-w-[300px] rounded-xl mx-auto md:mx-0" src={image} alt={title} />
        </div>

        <div className="flex-1 text-center md:text-left">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2">{title}</h1>
          <p className="font-semibold mb-4 sm:mb-5 border-b border-gray-200 pb-2 sm:pb-4">
            Developed by{" "}
          ...
          <div className="flex flex-col sm:flex-row justify-around md:justify-start gap-4 sm:gap-10 mb-5 text-center md:text-left">
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-bold">{downloads}M</h1>
            </div>
            <div>
          ...
          <button
            ...
            className={`btn font-semibold py-2 px-4 sm:py-3 sm:px-6 border-none rounded ${
      ...
      {/* BarChart */}
      <div className="max-w-[1250px] mx-auto mb-10 border-b border-gray-200 pb-10 sm:px-6 lg:px-8">
        <h1 className="text-xl sm:text-2xl font-bold mb-5 text-[#001931] text-center sm:text-left">
          Ratings
        </h1>
      ...
      <div className="max-w-[1250px] mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-5 text-[#001931] text-center sm:text-left">
          Description
        </h1>
        <p className="mb-6 sm:mb-10 text-[#627382] text-sm sm:text-base leading-relaxed text-justify sm:text-left">
          {description}
        </p>
      </div>
    </div>
